<?php
	class Database
	{
		...
		function GetSQLValueString($theValue, $theType,  $theNull="yes", $theDefinedValue = "", $theNotDefinedValue = "") 
		{
		  ...
		  // cooment 0n 08_dec
		  //$theValue = htmlentities(addslashes(rtrim($theValue)));
		  switch ($theType) {
			case "text":
		...
		//This function is to open the database connection
		{
			if (!defined("DB_SERVER_NAME") || !defined("DB_USERNAME") || !defined("DB_PASSWORD"))
				die("AdSpy Pro Error : Your copy of AdSpy Pro is not installed properly<br>Please install it again or check manual for details<br>Note: Re-Installation of software would remove all the data from tables<br> if you use same table prefix as used in past");
