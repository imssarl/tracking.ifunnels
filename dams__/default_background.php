...
{//alert(document.select_source_form.select_file.length);
	if(document.select_source_form.select_file.length==undefined)
	{ 
		window.opener.setBackgroundFile(field, document.select_source_form.select_file.value);
		self.close();
	}
	else
	{
		for(var i=0;i<document.select_source_form.select_file.length;i++)
		{
			...
			{
				window.opener.setBackgroundFile(field, document.select_source_form.select_file[i].value);
				self.close();
			}
		}
	}
...
<table width="100%" cellpadding="5" cellspacing="0" align="center" border="1" class="summary2">
	...
	<form name="select_source_form">
		<table width="100%" cellpadding="5" cellspacing="0" align="center" border="1">
				<?php
					$dir=ROOTPATH."background_images";
					if (is_dir($dir))
					{
						if ($dh = opendir($dir))
						{
							while (($file = readdir($dh)) !== false)
							{
								if(!is_dir($file))
								{
									...
									echo "<tr>";
										...
										echo "<td width='85%'><img width='100' src='background_images/$file'></td>";
