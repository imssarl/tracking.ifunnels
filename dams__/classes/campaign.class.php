...
session_start();
	class Campaign
	{
		...
		{	$position='';
			...
			if ($_POST['reset_css'] == 1) {
				$content = '<div class="cnmBanner">'.$content.'</div>';
				$content = $this->getCssStyle().$content;
			}
		...
		function update($id)
		{
			...
			if(isset($_POST["txt_contents"]) && $_POST["txt_contents"]!="")
			{
			 ...
			 if ($_POST['reset_css'] == 0  && $_POST['old_css'] == 1) { // Удалить css
			 	$content = preg_replace('@<style title=\\\"reset\\\"(.+)<span id=\\\"endstyle\\\"><\/span><div class=\\\"cnmBanner\\\">(.+)<\/div>@si','$2',$content);
			 }			 
			...
			{	
			 ...
			 if ($_POST['reset_css'] == 0  && $_POST['old_css'] == 1) { // Удалить css
			 	$fix_content = preg_replace('@<style title=\\\"reset\\\"(.+)<span id=\\\"endstyle\\\"><\/span><div class=\\\"cnmBanner\\\">(.+)<\/div>@si','$2',$fix_content);
			 }			 
			 if ($_POST['reset_css'] == 1 && $_POST['old_css'] != 1) { // Добавить css
			 	$fix_content = '<div class="cnmBanner">'.$fix_content.'</div>';	
			 	$fix_content = $this->getCssStyle().$fix_content;
...
/*			print_r($_POST);
	...
	//echo $sql;die();
		...
		function upload_background_img($campaign_id)
		{
			...
			$uploadfile = $uploaddir.$filename.'.'.$fileinfo['extension'];
...
//echo $sql;die();
		...
		function getCampaignOfHightestCTR($splitTestId)
		{
			...
			// get all add campaigns
			$sql = "SELECT campaign_id FROM `".TABLE_PREFIX."split_campaign` WHERE split_test_id=".$splitTestId;//WHERE campaign_id=".$splitTestId
			$campaignIDs = $damp_db->getData($sql);

			$CTR_array = array();
			foreach ($campaignIDs as $campaign)
			{
				// get each campaigns details
				$sql = "SELECT id,clicks,impression FROM `".TABLE_PREFIX."adcampaigns` where id=".$campaign['campaign_id'];
				$campaignDetails = $damp_db->getData($sql);
				$CTR_array[$campaignDetails[0]['id']] = 100*($campaignDetails[0]['clicks']/$campaignDetails[0]['impression']);
			}

			foreach ($CTR_array as $key => $val) {
			        if ($val == max($CTR_array)) return $key;
			}
