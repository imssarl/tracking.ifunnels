...
<tr>
	...
	<th>Template Used
		{if $arrPg.recall>1}
			{if $arrFilter.order!='temp_id--up'}<a href="{url name='site1_nvsb' action='manage' wg='order=temp_id--up'}"><img src="/skin/i/backend/up.gif" width="5" height="11" alt="" /></a>{else}<img src="/skin/i/backend/up_off.gif" width="5" height="11" alt="" />{/if}{if $arrFilter.order!='temp_id--dn'}<a href="{url name='site1_nvsb' action='manage' wg='order=temp_id--dn'}"><img src="/skin/i/backend/down.gif" width="5" height="11" alt="" /></a>{else}<img src="/skin/i/backend/down_off.gif" width="5" height="11" alt="" />{/if}
		{/if}
	</th>
	<th>Main Keyword
		{if $arrPg.recall>1}
...
<tr{if $k%2=='0'} class="matros"{/if}>
	...
	<td><img class="screenshot" rel="<div style='border:2px solid #000000;'><img src='{$preview}' /></div>" src="{img src=$preview w=95 h=60}{assign var=preview value=''}" /><br />{$arrTemplates[$v.temp_id].temp_name}</td>
	...
	<td class="option">
		<a href="{url name='site1_nvsb' action='edit'}?id={$v.id}" id="{$v.id}" rel="{$v.category_id}"><img title="Edit" src="/skin/i/frontends/design/buttons/edit.png" /></a>
		<a href="{url name='site1_nvsb' action='edit'}?id={$v.id}&template=change"><img title="Change template" src="/skin/i/frontends/design/buttons/template.png" /></a>
		<a href="{url name='site1_nvsb' action='manage'}?del={$v.id}" id="delete"><img title="Delete" src="/skin/i/frontends/design/buttons/delete.png" /></a>
		<a href="{url name='site1_blogfusion' action='create'}?nvsb={$v.id}"><img title="Install Wordpress" src="/skin/i/frontends/design/buttons/wpinstall.gif" /></a>
		...
		<a href="{url name='site1_nvsb' action='log'}?id={$v.id}" rel="width:800,height:500"  title="URL Log" class="mb" >URL&nbsp;Log</a>
	</td>
</tr>
{/foreach}
</tbody>
