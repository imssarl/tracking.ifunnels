...
var placeDo = function(){
	...
	placeParam.each( function( v,i ) {
		Ids[i] = v.id;
		var p = new Element( 'p', { 'html':index+'. '+v.title+' ' } );
	...
	$$('a.content-delete-list').each( function( el ){
		Ids[i] = el.get('rel');
		i++;	
	});
	jsonIds = JSON.encode( Ids );
	$( 'jsonContentIds' ).value = JSON.encode( Ids );
	return true;
