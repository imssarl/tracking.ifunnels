...
<table width="100%">
	...
	<tr{if $k%2=='0'} class="matros"{/if}>
		...
		<td>{$i.title}</td>
		<td align="center">{if $i.flg_source == 6}{*$i.count_posted_rss_content*}/rss{else}{$i.count_posted_content}/{$i.count_content}{/if}</td>
		...
		{$contentTypeName}</td>
		<td align="center">
			<a href="{url name='site1_content' action='blog_create'}?id={$i.id}">Edit</a> | 
