...
<a href="{url name='site1_cnb' action='fronttemplates' wg='restore=default'}" rel="<font color='red'>Note:</font> it won't affect the themes you uploaded" class="tips">Restore default themes</a>
...
<table width="100%">
	<tr>
		...
		<th width="12%">Added
		{if count($arrList)>1}
			{if $arrFilter.order!='added--up'}<a href="{url name='site1_cnb'  action='fronttemplates' wg='order=added--up'}"><img src="/skin/i/backend/up.gif" width="5" height="11" alt="" /></a>{else}<img src="/skin/i/backend/up_off.gif" width="5" height="11" alt="" />{/if}{if $arrFilter.order!='added--dn'}<a href="{url name='site1_cnb'  action='fronttemplates' wg='order=added--dn'}"><img src="/skin/i/backend/down.gif" width="5" height="11" alt="" /></a>{else}<img src="/skin/i/backend/down_off.gif" width="5" height="11" alt="" />{/if}
		{/if}		
		</th>
	...
	<tr>
		...
		<td align="center">{$i.added|date_local:$config->date_time->dt_full_format}</td>
				<td align="center">
		{if $i.flg_belong==1}
		<a href="{url name='site1_cnb' action='fronttemplates'}?delete={$i.id}">Del</a> |
		<a href="{url name='site1_cnb' action='edit_templates'}?id={$i.id}" >Edit</a> |
		{/if}
