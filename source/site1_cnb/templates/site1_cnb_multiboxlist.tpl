...
<tr>
	...
	<th><input type="checkbox" id="select_all" {if $smarty.get.check == 1} style="display:none;" {/if}  /></th>
</tr>
</thead>
<tbody>
...
<input type="hidden" value="{$v.title}" id="title_{$v.id}" />
<tr{if $k%2=='0'} class="matros"{/if}>
	<td>{$v.title}</td>
	<td width="150">{if $v.category}{$v.category}{/if}</td>
	<td><a href="{$v.url}" target="_blank">{$v.url}</a></td>
	<td class="option" align="center">
	<input class="check_site" type="checkbox" {if $smarty.get.check == 1} style="display:none;" {/if} value="{$v.id}">
	</td>
</tr>
{/if}
{/foreach}
...
<div {if $smarty.get.check == 1} style="display:none;" {/if}  align="center"><p><input type="button" value="Choose" id="choose"></p></div>

{literal}
<script type="text/javascript">
var CONST_SITE_TYPE=4;
