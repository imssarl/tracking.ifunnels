...
class Core_Media_Ftp {
	...
	public function setMode( $mode=null ) {
		if ( !in_array( $mode, array( self::TMODE_ASCII, self::TMODE_BINARY ) ) ) {
			return $this->setError( 'transfermode has either to be Core_Media_Ftp::TMODE_ASCII or Core_Media_Ftp::TMODE_BINARY' );
		}
		$this->_mode=$mode;
		return true;
	}
	 ...
	 * the files (using Core_Media_Ftp::LS_FILES_ONLY), to list only directories (using
	 * Core_Media_Ftp::LS_DIRS_ONLY) or to show both (using Core_Media_Ftp::LS_DIRS_FILES, which is
	 * default).
	 *
	 * The 4th one is the Core_Media_Ftp::LS_RAWLIST, which returns just the array created by
	 ...
	 * Core_Media_Ftp::LS_DIRS_FILES the directories will be added above the files,
	...
	private function listAndParse( &$arrRes ) {
		...
		usort( $dirs_list, array( 'Core_Media_Ftp', 'sort' ) );
		usort( $files_list, array( 'Core_Media_Ftp', 'sort' ) );
	...
	/**
	 * последовательность всех сообщений Core_Media_Ftp
	...
	/**
	 * только ошибки полученные при исполнении Core_Media_Ftp
	 * включая все отнаследованные классы
	 *
	 * @return array
	 */
