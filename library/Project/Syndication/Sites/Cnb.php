 ...
 */
 ...
 /**
 * коннектор к сайтам типа CNB для размещения КТ на таких сайтах
 *
 * @category Project
 * @package Project_Syndication
 * @copyright Copyright (c) 2005-2010, web2innovation
 * @license http://opensource.org/licenses/ MIT License
 */
class Project_Syndication_Sites_Cnb extends Project_Sites_Type_Cnb implements Project_Syndication_Sites_Interface {
	...
	public static function getInstance() {
		if ( self::$_instance==NULL ) {
			self::$_instance=new Project_Syndication_Sites_Cnb();
		}
		return self::$_instance;
	}

	public function run() {
		$object = new Project_Sites( Project_Sites::CNB );
		...
		$_arrSite['arrCnb']['arrContent']=$_arrPosts;
		$object->setData( $_arrSite );
		// постинг
		if ( !$this->set( $object ) ){
			Project_Sites_Type_Cnb::$_lastUrls=array();
	...
	public function prepareSource() {
		$this->_dir='Project_Syndication_Sites_Cnb@prepareSource';
		if ( !Zend_Registry::get( 'objUser' )->prepareTmpDir( $this->_dir ) ) {
			$this->_errors[] = 'Process Aborted. Can\'t create dir Project_Syndication_Sites_Cnb@prepareSource';
			return false;
		}
		mkdir($this->_dir . 'datas' . DIRECTORY_SEPARATOR . 'articles' . DIRECTORY_SEPARATOR, 0755, true );
