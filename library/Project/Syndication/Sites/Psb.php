 ...
 */
 ...
 /**
 * коннектор к сайтам типа PSB для размещения КТ на таких сайтах
 *
 * @category Project
 * @package Project_Syndication
 * @copyright Copyright (c) 2005-2010, web2innovation
 * @license http://opensource.org/licenses/ MIT License
 */
class Project_Syndication_Sites_Psb extends Project_Sites_Type_Psb implements Project_Syndication_Sites_Interface {
	...
	public static function getInstance() {
		if ( self::$_instance==NULL ) {
			self::$_instance=new Project_Syndication_Sites_Psb();
		}
		return self::$_instance;
	}

	public function run() {
		$object = new Project_Sites( Project_Sites::PSB );
		...
		$_arrSite['arrPsb']['arrContent']=$_arrPosts;
		$object->setData( $_arrSite );
		// постинг
		if ( !$this->set( $object ) ){
			Project_Sites_Type_Psb::$_lastUrls=array();
	...
	public function delete(){
		...
		$_arrSite['arrPsb']['arrContent']=array($this->_content);
		$object->setData( $_arrSite );
		if ( !$this->set($object) ){
			return false;
		}
	...
	public function set( Project_Sites  $object ) {
		$this->data=new Core_Data( $object->data->setFilter( array( 'trim', 'clear' ) )->getRaw( 'arrPsb' ) );
	...
	public function prepareSource() {
		$this->_dir='Project_Syndication_Sites_Psb@prepareSource';
		if ( !Zend_Registry::get( 'objUser' )->prepareTmpDir( $this->_dir ) ) {
			$this->_errors[] = 'Process Aborted. Can\'t create dir Project_Syndication_Sites_Psb@prepareSource';
			return false;
		}
		$arrDirs = array( 'auctions', 'blogs', 'forums', 'news', 'podcasts', 'press', 'products', 'videos' );
		mkdir($this->_dir . 'cache' . DIRECTORY_SEPARATOR);
		foreach ( $arrDirs as $dir ){
			mkdir($this->_dir . 'cache' . DIRECTORY_SEPARATOR . $dir .DIRECTORY_SEPARATOR );
		}
