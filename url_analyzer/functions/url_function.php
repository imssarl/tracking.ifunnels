...
{   
   ...
   // $varString=RemoveScriptTags($para,$para);
	...
	{   
	   ...
	   // commented if((trim($varRikin1)!="") && (trim($varRikin1)!=" ") && (trim(strlen($varRikin1))>=$txt_DontShowLessWords))

		if((trim($varRikin1)!="") && (trim($varRikin1)!=" "))
...
function Total_WordCount_With_Words($para,$txt_DontShowLessWords) 
   ...
   $i=1;
...

	{
		...
		{ 
			...
			$varStart=@strpos(strtolower($para), ">",$varcurpos);

			$varEnd=@strpos(strtolower($para), "</h$i>",$varStart+strlen(">"));

			$varHText= $varHText." ".trim(strip_tags(substr($orgcontents,($varStart+1),(($varEnd-$varStart))-1)));
...
$varStr = ereg_replace("[^a-z][0-9]]", " ", $varStr);
...
function Twowordphrases($para,$txt_DontShowLessWords,$NoOfwords)

{
...

	{
		...
		if (strlen(trim($arrOfContents[$i]))>0 && $NoOfwords==2)
		...
		if (strlen(trim($arrOfContents[$i]))>0 && $NoOfwords==3)

		{

						$varSeekStr=$arrOfContents[$i]." ".$arrOfContents[$i+1]." ".$arrOfContents[$i+2];
		...
		if (strlen(trim($arrOfContents[$i]))>0 && $NoOfwords==4)

		{

						$varSeekStr=$arrOfContents[$i]." ".$arrOfContents[$i+1]." ".$arrOfContents[$i+2]." ".$arrOfContents[$i+3];
